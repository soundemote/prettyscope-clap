...
try {
    ...
    if ($incompleteReports.Count -gt 0) {
        ...
        Write-Host ""
        Write-Host "Answer sheet commands:"
        Write-Host "  powershell -ExecutionPolicy Bypass -File .\scripts\new-daw-test-answer-sheet.ps1"
        Write-Host "  powershell -ExecutionPolicy Bypass -File .\scripts\apply-daw-test-answer-sheet.ps1 -AnswerPath `"path\to\prettyscope-daw-test-answer-sheet.json`" -RequireComplete"
        Write-Host ""
